...
- **`height`**: Present
  ...
  - Compiled to Vega v6 format
  - Verified in: Test output shows `height: 450`
...
- **Direct Vega Spec Response**: API returns Vega spec at root level
  - API Response Structure (Vega v6 specification):
    ```json
    {
...
The API returns the Vega v6 specification directly as the response body:
...
**Important**: The API returns the Vega specification directly, not wrapped in any container object. This is a pure Vega v6 visualization specification that can be used immediately with any Vega renderer.
...
- Vega-Lite: `width: 800, height: 450`
- Compiled Vega: `"width": 800, "height": 450` (at root level)
...
- Are returned directly as the API response (no wrapper objects)
- Compile correctly from Vega-Lite to Vega v6
- Pass all validation tests
- Render correctly in Vega viewers and editors

The API returns a complete, standalone Vega v6 specification that can be used immediately with any Vega-compatible visualization tool.
